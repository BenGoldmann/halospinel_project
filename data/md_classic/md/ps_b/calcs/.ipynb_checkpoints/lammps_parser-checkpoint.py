...
def split_traj(lammps_raw_loc):
    ...
    frames = range(50000, 20051000, 1000)
    counter = 0
...
def parse_step(lines, intial_line=0):
    ...
    if "ITEM: BOX BOUNDS" not in lines[4]:
        raise IOError("expected line {} to be BOX BOUNDS".format(intial_line + 4))
...
if __name__ == '__main__':
    ...
    li1 = Structure.from_spacegroup(sg='Fd-3m:2', lattice = lattice, species=['Li'], coords=[[0.125, 0.125, 0.125]]) #8a
    li2 = Structure.from_spacegroup(sg='Fd-3m:2', lattice = lattice, species=['Li'], coords=[[0.000, 0.000, 0.000]]) #16c
    li3 = Structure.from_spacegroup(sg='Fd-3m:2', lattice = lattice, species=['Li'], coords=[[0.125, 0.125, 0.875]]) #48f
    li4 = Structure.from_spacegroup(sg='Fd-3m:2', lattice = lattice, species=['Li'], coords=[[0.500, 0.500, 0.500]]) #16d
    li5 = Structure.from_spacegroup(sg='Fd-3m:2', lattice = lattice, species=['Li'], coords=[[0.125, 0.125, 0.625]]) #8b
    ...
    for strc in li_structures.values():
        strc.make_supercell([8,8,8])
        
    li1_sites = [VoronoiSite(s.frac_coords, label='Li1') for s in li1] #8a
    li2_sites = [VoronoiSite(s.frac_coords, label='Li2') for s in li2] #16c
    li3_sites = [VoronoiSite(s.frac_coords, label='Li3') for s in li3] #48f
    li4_sites = [VoronoiSite(s.frac_coords, label='Li4') for s in li4] #16d
    li5_sites = [VoronoiSite(s.frac_coords, label='Li5') for s in li5] #8b
    sites = li1_sites + li2_sites + li3_sites + li4_sites + li5_sites
    
    # Create atoms for site-analysis.
    start_structure = Structure.from_file('red.cif')
    ...
    with Pool(processes=48) as pool:
        ...
        process = pool.imap(create_traj, files[0::100)
        
        for strc, t_s in tqdm(process, total=len(files[0::100])):
